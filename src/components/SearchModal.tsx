...
const SearchModal = ({ isOpen, onClose }: SearchModalProps) => {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      ).slice(0, 6); // Reduced from 8 to 6 results
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/20 backdrop-blur-sm animate-fade-in">
      <div className="absolute top-[104px] left-1/2 transform -translate-x-1/2 w-full max-w-xl mx-auto px-4">
        <div className="bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden animate-scale-in">
          {/* Search Input */}
          ...
          {searchTerm.trim() !== '' && (
            <div className="border-t border-gray-100 max-h-64 overflow-y-auto">
              {loading ? (
                ...
                <div className="py-2">
                  <p className="text-sm text-gray-500 px-4 py-2 font-hm-sans">
                  ...
                  {searchResults.map((product, index) => (
                    ...
                    >
                      <img
                        src={product.image}
                        alt={product.name}
                        className="w-10 h-10 object-cover rounded-lg bg-gray-100 group-hover:scale-105 transition-transform"
                        onError={(e) => {
                          e.currentTarget.src = 'https://images.unsplash.com/photo-1594938298603-c8148c4dae35?w=400&h=500&fit=crop';
                        }}
                      />
                      <div className="flex-1 min-w-0">
                        <h4 className="font-medium text-gray-900 truncate font-hm-sans group-hover:text-black text-sm">
                          {product.name}
                        </h4>
          ...
          {searchTerm.trim() === '' && recentSearches.length > 0 && (
            <div className="border-t border-gray-100 py-3">
              <p className="text-sm text-gray-500 px-4 mb-2 flex items-center gap-2 font-hm-sans">
              ...
              <div className="space-y-1">
                {recentSearches.slice(0, 3).map((term, index) => (
                  <button
                    ...
                    className="block w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-50 transition-colors font-hm-sans text-sm"
                  >
                    {term}
                  </button>
                ))}
          ...
          {searchTerm.trim() === '' && recentSearches.length === 0 && (
            <div className="border-t border-gray-100 py-4">
              <p className="text-sm text-gray-500 px-4 mb-2 font-hm-sans">Recherches populaires:</p>
              <div className="px-4 flex flex-wrap gap-2">
                {["Costume", "Chemise", "Cravate", "Veste"].map((suggestion) => (
                  <button
                    key={suggestion}
                    onClick={() => setSearchTerm(suggestion)}
                    className="text-xs bg-gray-100 hover:bg-gray-200 px-2 py-1 rounded-full transition-colors font-hm-sans text-gray-700 hover:text-black"
                  >
                    {suggestion}
                  </button>
                ))}
